    ...
    return {
        importAssetFromHttpRequest: function(options) {
            ...
            if(!options.hasOwnProperty('attributes')){
                options.attributes ={};
                log.info("Has Own properties");
            }
            log.info("importAssetFromHttpRequest:"+options.path);
            return options;
        },
        ...
        create: function(options) {
            ...
            var parentPath = "/_system/governance/trunk/javascripts/".concat(version);
            var mediaType = "application/javascript";
        ...
        update: function(options){
            var log = new Log();
            var updatedContent = options.javascriptcontent;
            if(!updatedContent){
                log.info('undefined');
            }else{
                log.info(options);
                var name = options.overview_name;
                var version = options.overview_version;
                var resourcePath = "/_system/governance/trunk/javascripts/"+version+"/"+name;
                var userRegistry = getRegistry(ctx.session);
                var resource = userRegistry.registry.get(resourcePath);
                resource.setContent(updatedContent);
                userRegistry.registry.put(resourcePath, resource);
                log.info('JavaScript successfully updated'); 
            }
        },
