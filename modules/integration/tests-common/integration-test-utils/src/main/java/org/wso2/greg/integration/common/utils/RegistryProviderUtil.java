...
*/
    ...
    public RemoteRegistry getRemoteRegistry(String productGroup, String instance, String tenantDomain, String userKey)
            throws MalformedURLException, RegistryException, XPathExpressionException {
        String registryURL;
  ...
  /*      if (env.getFrameworkSettings().getEnvironmentSettings().is_runningOnStratos()) {
        ...
        try {
            registry = new RemoteRegistry(new URL(automationContext.getContextUrls().getServiceUrl()), automationContext.getUser().getUserName(), automationContext.getUser().getPassword());
        } catch (RegistryException e) {
