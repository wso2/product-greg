...
public class SampleDataPopulatorTestCase extends GREGIntegrationBaseTest {
    ...
    public void initTest() throws Exception {
        ...
        File file = new File(CarbonUtils.getCarbonHome() + File.separator + "samples" + File.separator + "asset-models" +
                File.separator + "FooModel");

        if (!file.exists())
            file.mkdirs();

        FileManipulator.copyDir(new File(FrameworkPathUtil.getSystemResourceLocation() + "artifacts" + File.separator
                                + "GREG" + File.separator +  "populate" + File.separator + "FooModel"),
                                new File(CarbonUtils.getCarbonHome() + File.separator + "samples" + File.separator + "asset-models" +
                                         File.separator + "FooModel"));

        FileManipulator.copyFileToDir(new File(FrameworkPathUtil.getSystemResourceLocation() + "artifacts" + File.separator
                                                + "GREG" + File.separator  + "populate" + File.separator +
                                               "build.xml"), new File(CarbonUtils.getCarbonHome() + File.separator + "samples" + File.separator +
                                                                      "asset-models" + File.separator + "Populator"));

        List<File> files = getAllSvnFiles(new File(CarbonUtils.getCarbonHome() + File.separator + "samples" + File.separator +
                                                   "asset-models" + File.separator + "Populator"));

        List<File> filesFoo = getAllSvnFiles(new File(CarbonUtils.getCarbonHome() + File.separator + "samples" + File.separator + "asset-models" +
                                                      File.separator + "FooModel"));
    ...
    public void dataUploadTest() throws RegistryException {
        assertTrue(wsRegistry.resourceExists("/_system/governance/trunk/wsdls/net/webservicex/www/globalweather.asmx.wsdl"),
                "Resource not found.");
    ...
    public void lifecycleOperationTest() throws RegistryException {
        assertTrue(wsRegistry.resourceExists("/_system/governance/branches/testing/services/net/webservicex/www/1.0.0/GlobalWeather"),
                   "Resource not found.");
        assertEquals(wsRegistry.get("/_system/governance/branches/testing/services/net/webservicex/www/1.0.0/GlobalWeather").getProperty(
