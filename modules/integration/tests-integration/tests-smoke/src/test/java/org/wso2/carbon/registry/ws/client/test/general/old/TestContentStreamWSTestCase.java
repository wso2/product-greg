 ...
 */
public class TestContentStreamWSTestCase extends GREGIntegrationBaseTest {
    ...
    public void testPutResourceAsStreamXML() throws Exception {
        ...
        InputStream is = new BufferedInputStream(new FileInputStream(getTestResourcePath() + "pom.xml"));
        String st = null;
        try {
            st = slurp(is);
        } catch(IOException e) {
        ...
        assertEquals(new String((byte[]) r2.getContent()), new String((byte[]) resource.getContent()),
    ...
    public void contentStreaming() throws Exception {
        ...
        assertEquals(new String((byte[]) r4.getContent()), new String((byte[]) r3.getContent()), "Content is not equal.");
        InputStream isTest = r4.getContentStream();
        assertEquals(convertStreamToString(isTest), new String((byte[]) r3.getContent()), "Content stream is not equal.");
        r3.discard();
    }
