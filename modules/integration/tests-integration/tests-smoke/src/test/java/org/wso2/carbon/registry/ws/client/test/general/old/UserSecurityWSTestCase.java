 ...
 */
public class UserSecurityWSTestCase extends GREGIntegrationBaseTest {

    private Registry everyOneRegistry = null;
    private WSRegistryServiceClient registry;
    public static String[] USER1 = {"userSecurityUN"};
    ...
    public void init() throws Exception {
        ...
        UserManagementClient adminUserManagementClient = new UserManagementClient(backendURL, getSessionCookie());

        adminUserManagementClient.addRole("user1Role", USER1, USER1_PERMISSION);

        adminUserManagementClient.updateUserListOfRole(FrameworkConstants.ADMIN_ROLE, null,
                USER1);//delete user from admin role

        AutomationContext automationContextUser = new AutomationContext("GREG", "greg001",
                FrameworkConstants.SUPER_TENANT_KEY, USER1[0]);

        everyOneRegistry = registryProviderUtil.getWSRegistry(automationContextUser);
    }
