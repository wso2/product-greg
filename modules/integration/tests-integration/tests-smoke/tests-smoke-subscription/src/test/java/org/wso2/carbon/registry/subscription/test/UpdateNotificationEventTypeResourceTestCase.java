...
public class UpdateNotificationEventTypeResourceTestCase extends GREGIntegrationBaseTest {
    ...
    public void testAddResource(String path) throws MalformedURLException, RemoteException, ResourceAdminServiceExceptionException {
        ...
        assertTrue(resourceAdminServiceClient.getResource(path + RESOURCE_NAME)[0].getAuthorUserName().contains(userNameWithoutDomain));
    }
    ...
    public void testAddResourceForAssociation() throws MalformedURLException, RemoteException, ResourceAdminServiceExceptionException {
        ...
        DataHandler dh = new DataHandler(new URL("file:///" + resourcePath));
        resourceAdminServiceClient.addResource(ROOT + ASSOCIATION_RESOURCE_NAME, "test/plain", "testDesc", dh);
        assertTrue(resourceAdminServiceClient.getResource(ROOT + ASSOCIATION_RESOURCE_NAME)[0].getAuthorUserName().contains(userNameWithoutDomain));
    }
    ...
    public void testConsoleSubscription(String path) throws Exception {
        infoServiceAdminClient = new InfoServiceAdminClient(backendURL, sessionID);
        SubscriptionBean bean = infoServiceAdminClient.subscribe(path + RESOURCE_NAME,
                "work://RoleSubscriptionTest", "ResourceUpdated", sessionID);
    ...
    public void testAddCommenttoResource(String path) throws Exception {
     ...
     * @throws RegistryExceptionException
     */
    @Test(groups = "wso2.greg", description = "Add tag", dependsOnMethods = "testAddCommenttoResource",
            dataProvider = "SubscriptionPathDataProvider")
    public void testAddTagtoResource(String path) throws RegistryException, AxisFault, RegistryExceptionException {
        infoServiceAdminClient.addTag(TAG, path + RESOURCE_NAME, sessionID);
        String tag = infoServiceAdminClient.getTags(path + RESOURCE_NAME, sessionID).getTags()[0].getTagName();
     ...
     * @throws RegistryExceptionException
     */
    @Test(groups = "wso2.greg", description = "Add rating", dependsOnMethods = "testAddTagtoResource",
            dataProvider = "SubscriptionPathDataProvider")
    public void testAddRatingtoResource(String path) throws RegistryException, RegistryExceptionException {
        infoServiceAdminClient.rateResource("1", path + RESOURCE_NAME, sessionID);
        int userRating = infoServiceAdminClient.getRatings(path + RESOURCE_NAME, sessionID).getUserRating();
    ...
    public void testAddDependencytoResource(String path) throws RemoteException, AddAssociationRegistryExceptionException {
        String dependencyType = "depends";
        String todo = "add";
        relationAdminServiceClient.addAssociation(path + RESOURCE_NAME, dependencyType, ROOT + ASSOCIATION_RESOURCE_NAME, todo);
    ...
    public void testAddAssociationtoResource(String path) throws RemoteException, AddAssociationRegistryExceptionException {
        String associationType = "association";
        String todo = "add";
        relationAdminServiceClient.addAssociation(path + RESOURCE_NAME, associationType, ROOT + ASSOCIATION_RESOURCE_NAME, todo);
        AssociationTreeBean aTreeBean = relationAdminServiceClient.getAssociationTree(path + RESOURCE_NAME, "association");
        assertTrue(aTreeBean.getAssociationTree().contains(ROOT + ASSOCIATION_RESOURCE_NAME), "Association is not correct");
    }
    ...
    public void testGetNotifications(String path) throws RemoteException, AddAssociationRegistryExceptionException, InterruptedException, IllegalStateFault, IllegalAccessFault, IllegalArgumentFault {
        ...
        for(WorkItem workItem : workItems) {
            if(workItem.getPresentationSubject().toString().contains("The tag " + TAG + " was applied on resource " + path +
                    RESOURCE_NAME)) {
                notiTag = true;
            } else if(workItem.getPresentationSubject().toString().contains("A rating of 1 was given to the resource at " + path +
                    RESOURCE_NAME)) {
                notiRate = true;
            } else if(workItem.getPresentationSubject().toString().contains("A comment was added to the resource at " + path +
