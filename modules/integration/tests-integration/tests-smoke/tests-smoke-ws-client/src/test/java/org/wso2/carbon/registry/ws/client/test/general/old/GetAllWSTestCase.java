...
public class GetAllWSTestCase extends GREGIntegrationBaseTest{
    ...
    public void initGetAllTest() throws Exception {
    ...
    public void testGetAllForResource() throws Exception {
        ...
        Resource resource;
        resource = WSRegistryClientUtils.transformWSResourcetoResource(registry, resourceData.getResource(), null);
        ...
        assertEquals("some comment", resourceData.getComments()[0].getText(), "Invalid comment text");
        assertEquals(1, resourceData.getAssociations().length, "Invalid association length");
        assertEquals(PARENT_PATH + "/mytext2.txt", resourceData.getAssociations()[0].getDestinationPath(), "Invalid destination path");
        assertEquals(PATH, resourceData.getAssociations()[0].getSourcePath(), "Invalid source path");
        assertEquals("relatesTo", resourceData.getAssociations()[0].getAssociationType(), "Invalid association type");
    }
    ...
    public void testGetAllForCollection() throws Exception {
        ...
        assertNotNull(resourceData.getResource());
        Resource resource;
        resource = WSRegistryClientUtils.transformWSCollectiontoCollection(registry, (WSCollection) resourceData.getResource(), null);
        ...
        assertEquals(1L, resourceData.getTags()[0].getTagCount(), "Invalid tag count");
        assertEquals(1, resourceData.getComments().length, "Invalid comment length");
        assertEquals("some other comment", resourceData.getComments()[0].getText(), "Invalid comment text");
        assertEquals(1, resourceData.getAssociations().length, "Invalid association length");
        assertEquals(PARENT_PATH + "/mytext2.txt", resourceData.getAssociations()[0].getDestinationPath(), "Invalid destination path");
        assertEquals(PARENT_PATH, resourceData.getAssociations()[0].getSourcePath(), "Invalid source path");
        assertEquals("relatesTo", resourceData.getAssociations()[0].getAssociationType(), "Invalid association type");
    }
