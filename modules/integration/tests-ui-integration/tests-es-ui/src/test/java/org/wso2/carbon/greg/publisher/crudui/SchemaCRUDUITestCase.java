...
/***
 * This class have test methods for Create,Remove,Update,Delete Operations for Custom RXT
 */
public class SchemaCRUDUITestCase extends GREGIntegrationUIBaseTest {
    private static final Log log = LogFactory.getLog(SchemaCRUDUITestCase.class);
    private static final String SCHEMA_NAME = "library.xsd";
    ...
    @Test(groups = "wso2.greg", description = "Create a schema by url and validate its availability")
    public void testCreateAsset() throws MalformedURLException, XPathExpressionException {
        ...
        asset.createGenericAsset("https://raw.githubusercontent.com/wso2/wso2-qa-artifacts/master/automation-artifacts/"
                        + "greg/schema/library.xsd", uniqueName + SCHEMA_NAME, SCHEMA_VERSION,
                uiElementMapper.getElement("publisher.schemas"));
    }
    ...
    public void testClickAndValidate() {
        asset.clickOnContentTypeAsset(uniqueName + SCHEMA_NAME, uiElementMapper.getElement("publisher.schemas"));
        asset.validateDetailsGenericType(uniqueName + SCHEMA_NAME,
                uiElementMapper.getElement("publisher.general.name"));
        asset.validateDetailsGenericType(SCHEMA_VERSION, uiElementMapper.getElement("publisher.general.version"));
    }

    @Test(groups = "wso2.greg", description = "Delete a Schema and validate its availability",
    ...
    public void testDeleteAsset() throws MalformedURLException, XPathExpressionException {
        ...
        asset.deleteAndValidateAsset(uiElementMapper.getElement("publisher.schemas"), uniqueName + SCHEMA_NAME);
    }
