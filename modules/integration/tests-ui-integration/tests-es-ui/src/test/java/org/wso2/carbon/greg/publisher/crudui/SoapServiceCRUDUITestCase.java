...
/***
 * This class have test methods for Create,Retrieve,Remove,Update Operations for Soap Service
 */
public class SoapServiceCRUDUITestCase extends GREGIntegrationUIBaseTest {
    private static final Log log = LogFactory.getLog(SoapServiceCRUDUITestCase.class);
    ...
    @Test(groups = "wso2.greg", description = "Create soap service and validate its availability")
    public void createAsset() throws MalformedURLException, XPathExpressionException {
        uniqueName = getUniqueName();
        asset.createGenericTypeAsset(uniqueName, "/" + uniqueName, SERVICE_VERSION, "TestSoap1" + uniqueName,
                uiElementMapper.getElement("publisher.soapservices"));
        elementID = asset.getUUID();
        driver.findElementWD(By.linkText(uiElementMapper.getElement("publisher.soapservices"))).click();
    }
    ...
    @Test(groups = "wso2.greg", description = "Update soap service and validate it",
            dependsOnMethods = "clickAndValidate")
    public void updateAsset() throws MalformedURLException, XPathExpressionException {
        uniqueName = getUniqueName();
        asset.updateAsset(uiElementMapper.getElement("publisher.soapservices"), log.getClass().getName(), elementID,
                uniqueName);
    }
    ...
    @Test(groups = "wso2.greg", description = "Delete soap service and validate its availability",
            dependsOnMethods = "clickAndValidateAfterUpdate")
    public void deleteAsset() throws MalformedURLException, XPathExpressionException {
        PublisherUtil asset = new PublisherUtil(driver);
        asset.clickOnGenericTypeAsset(uiElementMapper.getElement("publisher.soapservices"), elementID);
        asset.deleteAndValidateAsset(uiElementMapper.getElement("publisher.soapservices"), elementID);
    }
